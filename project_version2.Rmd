...
#dropping CALC as it contains only 0
train1 <- subset(train, select=-c(CALC))
...
#train1$NObeyesdad <- mapvalues(train1$NObeyesdad,
 ...
 svm1 <- svm(NObeyesdad~., data=train1, 
...
```{r}
#training prediction
svm.predtrain <- predict(svm1, train1)
xtab.train <- table(train1$NObeyesdad, svm.predtrain)
print("Confussion matrix for train data")
xtab.train

#test prediction
test1 <- subset(test, select=-c(CALC))
svm.predtest <- predict(svm1, test1)
xtab.test <- table(test1$NObeyesdad, svm.predtest)
...
print(paste("Misclassification error rate in train = ", round(svm.train.error*100,4),"%"))
print(paste("Misclassification error rate in test = ", round(svm.test.error*100,4),"%"))

```
```{r}
plot(svm1,test1,Age~Height)
```
...
xtab.train <- table(train1$NObeyesdad, bag.predtrain)
print("Confussion matrix for train data")
xtab.train
...
bag.predtest <- predict(gbag, test)
xtab.test <- table(test1$NObeyesdad, bag.predtest)
print("Confussion matrix for test data")
xtab.test
...
print(paste("Misclassification error rate in train = ", round(bag.train.error*100,4),"%"))
print(paste("Misclassification error rate in test = ", round(bag.test.error*100,4),"%"))
```
...
varimp.data <- subset(data.final, select=-c(NObeyesdad))
...
```{r}
#training prediction
gbm.predtrain <- predict(boost, train1)
xtab.train <- table(train1$NObeyesdad, gbm.predtrain)
print("Confussion matrix for train data")
xtab.train
...
print(paste("Misclassification error rate in train = ", round(gbm.train.error*100,4),"%"))
print(paste("Misclassification error rate in test = ", round(gbm.test.error*100,4),"%"))
```
...
writeLines("--- Training Error - Random Forest ---")
print(mean(rf.pred != train$Purchase))

table(rf.pred, train$Purchase)
