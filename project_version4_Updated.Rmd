...
4 - Relationship between Weight & Height for both genders

Modeling - Support Vector Machines & Bagging (for dataset with BMI)
Support Vector Machines & Bagging (for dataset without BMI)

PradeepSurya

Data Cleaning & Preparation
Data Encoding
Replacing categorical values of target variable to numeric
...
```{r}
roc_svm_test <- roc(response = test$NObeyesdad, predictor =as.numeric(test.pred))
...
```
```{r}
plot(svm1,test,Age~BMI)
```

# Tuning SVM
```{r}
train$NObeyesdad <- as.factor(train$NObeyesdad)
tune.svm <- tune(svm, NObeyesdad~., data=train, kernel="radial",type="C-classification",ranges=list(cost=2^(-3:2),gamma=2^(-25:1)), tunecontrol = tune.control(nrepeat = 5, sampling = "cross", cross = 5))
```
...
train1$NObeyesdad <- mapvalues(train$NObeyesdad,
...
```{r}
# Train
new_df <- data.frame(train.pred, tree.predtrain, train$NObeyesdad)
...
```{r}
new_df <- data.frame(test.pred, tree.predtest, test$NObeyesdad)
...
Calculating variable importance in SVM

```{r}
library(rminer)
train_new$NObeyesdad<-as.numeric(train_new$NObeyesdad)
model<- fit(NObeyesdad~.,data = train_new, model ="svm", kpar= list(sigma=0.1), C=2)
svm.imp<- Importance(model,data=train_new)
svm.imp$imp
```
...
gbag_new <- bagging(as.factor(NObeyesdad) ~ ., data = train1_new,coob=T,nbag=100)
print(gbag_new)
```
The Out-of-bag estimate error increased from 4% to 19.55% by elimating BMI & Weight from the dataset
